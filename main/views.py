...
def profile_update(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid:
            ...
            new = user.first_name.title()
            messages.error(request, f'dear {new} your profile is update generated the following errors: {form.errors}')
...
def delete(request):
    if request.method == 'POST':
        ...
        messages.success(request, 'one item dleted from cart')
        return redirect('cart') 
...
def payment(request):
    if request.method == 'POST':
        ...
        api_key = 'sk_test_f30fe959db9d97913bea665d263eda3e3491004f' #secret key from paystack
        curl = 'https://api.paystack.co/transaction/initialize' #paystack call url
        cburl = 'http://13.50.224.188/callback' #payment confirmation page
